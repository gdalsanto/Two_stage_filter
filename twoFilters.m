function [Hatt, w, target_mag, h, SOS, pads] = twoFilters(RT, dL, fs, method, wc) %, varargin)
...
%% input parser 
p = inputParser; 
addRequired(p, 'RT', @(x) isnumeric(x));
addRequired(p, 'dL', @(x) isnumeric(x) && length(x) == 1);
addRequired(p, 'fs', @(x) isnumeric(x));
addRequired(p, 'method', @(x) any(validatestring(x, {'GEQ', 'median', 'notch', 'shelf'})));
addRequired(p, 'wc', @(x) isnumeric(x));

% TODO adjust functions to accepts arbitrary central frequencies 
% addOptional(p,'f',[],  @(x) assert(length(x) == length(RT)));    
% parse(p,RT, dL, fs, method, wc, varargin{:});

%% step zero - preprocess the input RT vector
...
target_mag(ind(1):ind(end)) = interp1(f, gdB_dl, w(ind(1):ind(end)), "linear", "extrap"); % interpolate linearly between known points NOTE extrapolate is needed in those cases when w(ind(1)) is > f(1)
...
legend('target mag', 'shelf', 'difference mag', 'H attenuation', 'H GEQ')
xlim([1 fs/2])
